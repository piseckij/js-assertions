...
function unbracketTag(str) {
    ...
    return str.substring(0, first) + str.substring(first + 1, last) + str.substring(last + 1, str.length);
}
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        let index;
        if ((index = upper.indexOf(char)) !== -1) {
            result += upper.charAt((index + 13) % 26);
        } else if ((index = lower.indexOf(char)) !== -1) {
            result += lower.charAt((index + 13) % 26);
        } else {
            result += char;
        }
    }
...
function getCardId(value) {
    ...
    return cards.indexOf(value.substring(0, value.length - 1))
        + cards.length * signs.indexOf(value.substring(value.length - 1, value.length));
}
